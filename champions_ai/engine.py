...
MOVE_POWER_DB = {"じしん":100,"りゅうせいぐん":130,"げきりん":120,"アイアンヘッド":80,"かえんほうしゃ":90,"だいもんじ":110,"ハイドロポンプ":110,"れいとうビーム":90,"シャドーボール":80,"10まんボルト":90,"ボルテッカー":120,"インファイト":120,"ラスターカノン":80,"りゅうのはどう":85}
TYPE_EFFECT = {("じめん","でんき"):2.0,("じめん","ほのお"):2.0,("じめん","いわ"):2.0,("じめん","はがね"):2.0,("じめん","くさ"):0.5,("じめん","むし"):0.5,("じめん","ひこう"):0.0,("ドラゴン","ドラゴン"):2.0,("フェアリー","ドラゴン"):2.0,("ほのお","くさ"):2.0,("みず","ほのお"):2.0,("でんき","みず"):2.0}
POKEMON_TYPES = {"ガブリアス":["ドラゴン","じめん"],"フラエッテ:永遠":["フェアリー"],"リザードン":["ほのお","ひこう"],"アシレーヌ":["みず","フェアリー"],"カバルドン":["じめん"],"ギャラドス":["みず","ひこう"],"ルカリオ":["かくとう","はがね"],"ゲンガー":["ゴースト","どく"],"ブリジュラス":["はがね","ドラゴン"]}
...
class ChampionsAI:
    ...
    def set_strength_profile(self, profile: str) -> None:
        profile = (profile or "standard").lower()
        if profile == "pokechamp":
            self.search_depth = 4
            self.rollout_count = 128
            self.cache.max_size = 30000
        elif profile == "high":
            self.search_depth = 4
            self.rollout_count = 64
            self.cache.max_size = 20000
        elif profile == "fast":
            self.search_depth = 2
            self.rollout_count = 16
            self.cache.max_size = 8000
        else:
            self.search_depth = 3
            self.rollout_count = 24
            self.cache.max_size = 12000
    ...
    def suggest(self, st: BattleState, top_k: int = 5) -> List[Tuple[Action, float, str]]:
        random.seed(f"{st.my_active}|{st.opp_active}|{st.turn}")
    ...
    def _expected_damage(self, atk_name: str, def_name: str, action: Action, attacker: Optional[PokemonState]=None) -> float:
        ...
        move_rows = usage.get("sections", {}).get("技", {}).get("moves", [])
        u = {m.get("name"): m.get("rate", 0.0) for m in move_rows}
        move_type = next((m.get("type") for m in move_rows if m.get("name")==action.name), "")
        p = self._pokechamp_move_prior(atk_name)
        blended = 0.55 * u.get(action.name, 8.0) + 0.45 * p.get(action.name, 8.0)
        power = MOVE_POWER_DB.get(action.name, 70)
        dmg = 8.0 + 0.28 * power + 0.35 * blended
        atk_types = POKEMON_TYPES.get(atk_name, [])
        def_types = POKEMON_TYPES.get(def_name, [])
        if move_type and move_type in atk_types:
            dmg *= 1.5
        if move_type and def_types:
            mult = 1.0
            for t in def_types:
                mult *= TYPE_EFFECT.get((move_type, t), 1.0)
            dmg *= mult
        if any(k in action.name for k in ["まもる","ステルスロック","ちょうはつ","でんじは","つるぎのまい"]):
            dmg *= 0.35
        if attacker is not None:
            dmg *= self._mega_damage_bias(attacker, action.name)
        return max(0.0, min(95.0, dmg))
