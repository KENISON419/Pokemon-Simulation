...
from champions_ai import BattleState, PokemonState, Action
from pokemon_AI.pokechamp.champions_adapter import ChampionsPokechampAdapter
...
ADAPTER = ChampionsPokechampAdapter(ROOT, mode="pokechamp")
AI = ADAPTER.ai
...
def usage_options_for(name: str):
    ...
    ability_list = [m.get("name") for m in sec.get("特性", {}).get("abilities", []) if m.get("name")]
    opts = {"moves": moves[:30], "items": items[:30], "natures": natures[:30], "spreads": spreads[:30], "abilities": ability_list[:30]}
    opts["default"] = {
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/model_stats":
            return self._json(200, {"history_actions": len(AI.outcome_stats), "search_depth": AI.search_depth, "rollout_count": AI.rollout_count, "mode_hint": "pokechamp/high/standard/fast"})
        if p.startswith("/api/ai_mode"):
            qs = parse_qs(urlparse(self.path).query)
            mode = qs.get("mode", ["standard"])[0]
            ADAPTER.set_mode(mode)
            return self._json(200, {"ok": True, "mode": mode, "search_depth": AI.search_depth, "rollout_count": AI.rollout_count})
    ...
    def do_POST(self):
        ...
        if p == "/api/suggest":
            ...
            result = ADAPTER.suggest(data, top_k=int(data.get("top_k", 5)))
            sid = data.get("session_id", "adhoc")
            log_event(sid, "suggest", st.turn, {"my_active": st.my_active, "opp_active": st.opp_active, "top": [x.get("action",{}) for x in result.get("suggestions",[])[:3]], "opp_set": result.get("opponent_prediction", {})})
